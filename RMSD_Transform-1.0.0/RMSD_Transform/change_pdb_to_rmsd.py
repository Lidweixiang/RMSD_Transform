...
def change_pdb(default_pdb_path, rmsd_mutiple_dic, rmsd_max_plus_value,rmsd_pdb_path):
...
def change_pdb_all(rmsd_dic,times,default_pdb_path, rmsd_pdb_path,plus_value = 10):
    rmsd_mutiple_dic = change_rmsd_dic_range(rmsd_dic, times)
    rmsd_max_plus_value = get_max_rmsd(rmsd_dic, times, plus_value)
    change_pdb(default_pdb_path, rmsd_mutiple_dic, rmsd_max_plus_value,rmsd_pdb_path)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="This module is for change pdb b-factor to RMSD")
