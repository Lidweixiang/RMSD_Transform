    ...
    with open(result_log) as rl:
        for line in rl:
            if line.count("|") == 4:
                ...
                if (chunk_info[1].strip() and chunk_info[2].strip(
                ) and chunk_info[3].strip() and chunk_info[1].strip() != "Query"):
                    aa_info = chunk_info[1].strip().split(":")
                    aa_number = aa_info[0][-1] + "_" + \
                        "".join(aa_info[1].split())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="This module is for get rmsd value")
    parser.add_argument(
        "-l",
        "--log_file",
